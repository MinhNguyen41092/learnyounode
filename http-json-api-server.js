...
var server = http.createServer((request, response) => {
  var apiUrl = url.parse(request.url, true);
  var date = apiUrl.query.iso;
  date = new Date(date);
  ...
  if(apiUrl.pathname.includes("unixtime")) {
    jsonTime = {"unixtime": unix};
  } else {
    jsonTime = {
      "hour": hours,
      "minute": minutes,
  ...
  jsonTime = JSON.stringify(jsonTime);
  response.writeHead(200, { 'Content-Type': 'application/json' });
  response.end(jsonTime);
});
server.listen(port);
